...
export default function Home(){
    ...
    return(
        <header id='home' className="max-w-[1700px] mx-auto scroll-smooth h-auto flex bg-background px-14rem py-10rem flex-col gap-[8rem] max-xl:py-6rem max-xl:px-8rem max-lg:h-auto max-md:gap-14 max-sm:px-[4.5rem] max-xs:px-[2.5rem]">
            ...
            <section className='basis-full flex items-center gap-12 max-lg:flex-col-reverse'>
                <div className='basis-full flex flex-col gap-4 animate-fade-in-left max-lg:items-center justify-center'>
                    ...
                    <span className='font-poppins-light text-secondary-blue-2 text-[1.25rem] max-2xl:mb-7 max-2xl:text-[1.20rem] max-xl:text-[1.10rem] max-lg:text-center max-sm:text-[1rem]'>I’m a <span className='font-bold'>junior full-stack developer</span> based in Calauan, Laguna, Philippines. I have participated in technology-related curricular activities that focus on building technical and efficient applications, honing my skills in emerging technologies, server-side programming, and database management.</span>
                    <button onClick={() => setOpenResume(true)} className='h-auto w-auto px-10 self-end py-3 bg-white rounded-4xl font-poppins-medium max-2xl:self-end max-lg:self-center cursor-pointer hover:bg-white/85'>View my Resume</button>
                </div>
